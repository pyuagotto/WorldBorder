 ...
 */
export const worldborderAdd = function(ev){
    ...
    const array = message.trim().split(/\s+/);
        ...
        if(array.length === 1){
            ...
            if(!Number.isNaN(newDistance)){
                ...
                if(sourceEntity instanceof Player) sourceEntity.sendMessage(`§c構文エラー:§a/scriptevent "worldborder:add" <distance:value> [time:int]`);
            }
        }
